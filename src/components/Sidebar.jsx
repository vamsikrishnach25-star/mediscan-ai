...
const Sidebar = ({ sidebarOpen }) => {
  ...
  const linkClass = "flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition";
  const activeClass = "bg-blue-600 text-white";
  const inactiveClass = "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800";

  return (
    <aside className={`fixed md:static top-0 left-0 z-40 h-full w-64 bg-white dark:bg-gray-900 border-r dark:border-gray-800 transform ${
      sidebarOpen ? "translate-x-0" : "-translate-x-full"
    } md:translate-x-0 transition-transform duration-200`}>

      {/* Logo — clicking goes to /welcome */}
      <div
