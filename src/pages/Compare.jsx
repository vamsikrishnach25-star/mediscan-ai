...
const Compare = () => {
  ...
  const getComparisonIcon = (val1, val2) => {
    if (val1 < val2) return <TrendingUp size={16} className="text-red-500" />;
    if (val1 > val2) return <TrendingDown size={16} className="text-green-500" />;
    return <Minus size={16} className="text-gray-400" />;
  };

  const r1 = report1 ? getAnalysis(report1) : null;
  const r2 = report2 ? getAnalysis(report2) : null;

  const risk1 = report1 ? getRiskLevel(report1) : null;
  const risk2 = report2 ? getRiskLevel(report2) : null;
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">Select two reports to compare side by side</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
          ...
          {report1 && (
            <div className="mt-4 p-4 rounded-xl" style={{ background: `${getRiskColor(risk1)}15`, border: `1px solid ${getRiskColor(risk1)}40` }}>
              <p className="font-bold">{getReportType(report1)}</p>
              <p className="text-sm text-gray-500">{new Date(report1.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</p>
              <p className="font-bold mt-2" style={{ color: getRiskColor(risk1) }}>Risk: {risk1}</p>
              <p className="text-sm text-gray-500">{r1?.severity_score || 0} abnormal markers</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
          ...
          {report2 && (
            <div className="mt-4 p-4 rounded-xl" style={{ background: `${getRiskColor(risk2)}15`, border: `1px solid ${getRiskColor(risk2)}40` }}>
              <p className="font-bold">{getReportType(report2)}</p>
              <p className="text-sm text-gray-500">{new Date(report2.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</p>
              <p className="font-bold mt-2" style={{ color: getRiskColor(risk2) }}>Risk: {risk2}</p>
              <p className="text-sm text-gray-500">{r2?.severity_score || 0} abnormal markers</p>
            </div>
          )}
        </div>
      </div>
      ...
      {report1 && report2 && (
        <div className="space-y-6">
              ...
              <div className="text-center p-4 rounded-xl bg-gray-50 dark:bg-gray-700">
                ...
                <div className="mt-2 flex justify-center">
                  {(r2?.health_score ?? 0) < (r1?.health_score ?? 0) ? (
                    <span className="text-green-500 font-semibold text-sm">✅ Better score!</span>
                  ) : (r2?.health_score ?? 0) > (r1?.health_score ?? 0) ? (
                    <span className="text-red-500 font-semibold text-sm">⚠️ Worse score</span>
                  ) : (
                    <span className="text-gray-500 font-semibold text-sm">➡️ Same</span>
                  )}
                </div>
          ...
          {(r1?.biomarkers || r1?.highlighted_biomarkers) && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
          ...
          {/* Possible Conditions Comparison */}
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
              ...
              {r1?.possible_conditions?.length > 0 ? (
                ...
                <p className="text-sm text-green-500">✅ No conditions identified</p>
              )}
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
      ...
      {!report1 && !report2 && (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl shadow">
          ...
          <p className="text-gray-500">Choose reports from the dropdowns above to see a detailed comparison</p>
        </div>
      )}
